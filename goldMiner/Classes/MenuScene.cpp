...
bool MenuScene::init()
{
	...
	gold_miner->setPosition(Vec2(visibleSize.width / 2 + origin.x, visibleSize.height / 2 + origin.y + 180));
	this->addChild(gold_miner, 1);

	auto gold = Sprite::create("menu-start-gold.png");
	gold->setPosition(Vec2(visibleSize.width + origin.x - 210, 140 + origin.y));
	...
	start->setPosition(Vec2(visibleSize.width + origin.x - 205, 190 + origin.y));
